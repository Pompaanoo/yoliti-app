...
export default async function ProgresoPage() {
  ...
  const { data: progressRaw } = await supabase
    ...
    .select(
      "user_id, lesson_id, completed, progress_pct, completed_at, profiles(full_name), lessons(title, modules(title, courses(title)))"
    )
  ...
  const byUser: Record<
    string,
    { name: string; total: number; done: number; avgPct: number }
  > = {};
  for (const r of progress) {
    ...
    byUser[r.user_id].avgPct =
      Math.round(
        (byUser[r.user_id].avgPct * (byUser[r.user_id].total - 1) +
          r.progress_pct) /
          byUser[r.user_id].total
      );
  }

  // Grupos con sus estadísticas
  const { data: groupsRaw } = await supabase
    .from("groups")
    .select(
      "id, name, group_students(user_id), group_courses(course_id)"
    );

  const groups = (groupsRaw as {
    id: string;
    name: string;
    group_students: { user_id: string }[];
    group_courses: { course_id: string }[];
  }[]) ?? [];
  ...
  const groupProgress: GroupProgress[] = groups.map((g) => {
    ...
    const totalLessons = courseIds.reduce(
      (acc, cid) => acc + (lessonsByCourse[cid] ?? 0),
      0
    );
    const totalPossible = totalLessons * studentIds.length;

    const completed = progress.filter(
      (r) => studentIds.includes(r.user_id) && r.completed
    ).length;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold text-secondary">Progreso de alumnos</h1>
        <p className="mt-1 text-sm text-base-content/60">
          Visualiza el avance de cada alumno y grupo.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          ...
          {
            icon: "fa-chart-pie",
            label: "Tasa global de completado",
            value:
              totalProgress > 0
      ...
      {groupProgress.length > 0 && (
        <section className="rounded-box border border-base-300 bg-base-100 p-6 shadow-sm">
          <h2 className="mb-4 font-bold text-secondary">Avance por grupo</h2>
                ...
                <div key={g.id} className="rounded-xl border border-base-300 p-4">
                  ...
                  <div className="mt-1 flex gap-4 text-xs text-base-content/50">
                    ...
                    <span>
                      ...
                      {g.courseCount} cursos
                    </span>
                  </div>
                  <div className="mt-3">
                    <div className="mb-1 flex justify-between text-xs">
                      <span className="text-base-content/60">Completado</span>
      ...
      {/* Progreso por alumno */}
      <section className="rounded-box border border-base-300 bg-base-100 p-6 shadow-sm">
        <h2 className="mb-4 font-bold text-secondary">Detalle por alumno</h2>
      ...
      {/* Actividad reciente */}
      <section className="rounded-box border border-base-300 bg-base-100 p-6 shadow-sm">
        <h2 className="mb-4 font-bold text-secondary">Actividad reciente</h2>
        {progress.length === 0 ? (
          <p className="text-sm text-base-content/50">Sin actividad.</p>
        ) : (
          <ul className="divide-y divide-base-200">
            {progress.slice(0, 20).map((r, i) => (
              <li key={i} className="flex items-start gap-3 py-3">
                <span
                  className={`mt-0.5 grid h-7 w-7 flex-shrink-0 place-items-center rounded-full text-xs ${
                    r.completed
                ...
                >
                  <i
                    className={`fa-solid ${r.completed ? "fa-circle-check" : "fa-clock"}`}
                  />
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">
                    {r.profiles?.full_name ?? "Alumno"}{" "}
                    <span className="font-normal text-base-content/50">
                      {r.completed ? "completó" : "vio"}
                    </span>{" "}
                    <span className="text-secondary">{r.lessons?.title}</span>
                  </p>
                  <p className="text-xs text-base-content/40">
                    {r.lessons?.modules?.courses?.title} →{" "}
                    {r.lessons?.modules?.title}
                    {r.completed_at && (
